...
class HomeController extends Controller
{
    ...
    public function Get_Album_By_Video(Request $request)
    {
        try {
            ...
            $result = album::where('id',$album_id)->where('type',"video")->get();
    ...
    public function Premium_AI_Audio(Request $request)
    {
        try{
            ...
            $data = Audio::where('is_paid',"1")->where('is_aiaudiobook',"1")->with('category','artist','user')->latest();
    ...
    public function Most_Viewed_AI_Audio(Request $request)
    {
        try{
            ...
            $data = Audio::orderBy('v_view','DESC')->where('is_aiaudiobook',"1")->with('category','artist','user')->latest();
    ...
    public function ai_audio_by_id(Request $request)
    {
        try{
            ...
            $aiAudioTransaction = Audio_Transaction::where('aiaudio_id',$request['video_id'])->where('user_id',$request->user_id)->get();
            if ($data) {
                $ra = $data->toArray();
                $data1 = $this->common->get_all_count_for_ai_audio($ra['id'], $user_id, $ra['user_id']);
                ...
                $ra->flag = !empty($aiAudioTransaction) ? 1 : 0 ;
    ...
    public function AI_Audio_by_category(Request $request)
    {
        try {
            ...
            $data = Audio::where('category_id',$category_id)->where('is_aiaudiobook',"1")->with('user','category','artist')->latest();
    ...
    public function Get_Album_By_Audio(Request $request)
    {
        try {
            ...
            $result = album::where('id',$album_id)->where('type',"audio")->get();
    ...
    public function Get_Album_By_AI_Audio(Request $request)
    {
        try {
            ...
            $result = album::where('id',$album_id)->where('type',"aiaudio")->get();

            if(!empty(sizeof($result))){
                foreach ($result as $key => $value) {
                    $C_Ids = explode(',', $value['video_id']);
                }
                $data = Audio::whereIn('id', $C_Ids)->where('is_aiaudiobook',"1")->with('category','artist','user')->orderBy('created_at', 'desc');
    ...
    public function get_like_ai_audio(Request $request)
    {
        try {
            ...
            $data = Audio::whereIn('id', $Ids)->where('is_aiaudiobook',"1")->with('user','artist','category')->orderBy('created_at', 'desc');
